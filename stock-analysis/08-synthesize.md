...
Map to an analyst-style rating (avoids false binary):

| Rating | Meaning |
|---|---|
| **BUY** | Attractive fundamentals + valuation offers meaningful upside vs price target; risk/reward favorable |
...
2. **Run the governance gates in sequence (P6)** — deterministic, logged, non-negotiable:
   - **Quality Gate:** if BUY but the company is low quality (not profitable / weak balance sheet / no growth), demote to HOLD with reduced confidence.
...
5. **Fold in contested signals:** any unresolved document contradiction surfaced in Phase 7 caps confidence at MEDIUM or below and is stated in the recommendation rationale.
...
Back the qualitative confidence (0–1) with a small numeric score so it stays comparable across review cycles:

Score **0–10** across these five axes and map to conviction (HIGH/MEDIUM/LOW):

| Axis | What it measures |
|---|---|
| Source quality & completeness | primary vs secondary sources, gaps found in Phase 2 |
| Data consistency | cross-aggregator agreement, freshness, earnings reconciliation (Phase 3) |
| Valuation spread | width of the fair-value range / dispersion (Phase 4) |
| Technical alignment | technicals/regime confirming or contradicting the thesis (Phase 5) |
| Catalysts/risk clarity | how cleanly the falsification criteria can be tested |

Average (or weight, documenting the weights) and map: **HIGH ≥7.5**, **MEDIUM 5.0–7.4**, **LOW <5.0**. A wide valuation spread or an unreconciled earnings gap caps the score at MEDIUM or below. State the numeric score alongside the confidence/conviction in the note.
...
5. ✅ Confidence and falsification criteria are stated (with numeric rubric score); unresolved contested signals reflected
