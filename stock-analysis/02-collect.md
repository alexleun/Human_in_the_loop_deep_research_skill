...
6. **Primary-filing extraction fallback:** HKEXnews annual/interim PDFs are often not machine-extractable via plain text fetch. Attempt extraction in order: `pdftotext` / HTML-text fetch / PDF text via Python libs. If the filing is still not machine-readable, **log the substitution** in `sources_index.md` — which secondary aggregator was used for which figure and why (e.g. "not machine-readable → NewTimeSpace/S&P/stockanalysis"). A silent downgrade to aggregator-consensus is not acceptable; the fallback must be recorded. Follow the layered access ladder: HKEXnews PDF → pdftotext/HTML/Python extraction → aggregator substitute (logged) → news gateway last.
