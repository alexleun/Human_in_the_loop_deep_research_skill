...
| **stock-analysis** | `stock-analysis/` | v2.1 | The single canonical **single-stock** institutional analysis (HKEX-optimized): 9-phase evidence-first workflow with adversarial Bull/Bear debate, DCF + reverse-DCF + SOTP, fragility audit, deterministic governance gates, document intelligence, and a BUY / HOLD / SELL / AVOID / WATCH call |
...
### stock-analysis v2.1 Highlights

Refined from two live evolution logs (0001.HK CK Hutchison, 0700.HK Tencent): valuation conventions and definition-labeling discipline that turn noisy live-market data into a defensible target.

| Feature | What It Does |
|---|---|
| **Implied-Required-Return Reverse-DCF** | §2b reads the market through the **implied required return at 0% growth**; when the DCF value at 0% growth exceeds market price the read is "market applies a higher required return / holding discount". A growth-solver that pins at a bound is labeled **degenerate**. |
| **Median-of-Methods Target** | Default target = **median** of the SOTP/DCF/peer/DDM outputs (never an arbitrary blend); scenario weighting uses the same base-median method set after **floor/ceiling methods are excluded `(DDM floor, forced-bear cap)`** — the 0700 DDM-floor mis-price moved from −11% to +8% after the fix. |
| **Non-IFRS ↔ IFRS Bridge (First-Class)** | Item-by-item bridge (SBC, investee/associate contributions, intangible amortization, one-offs) with the driving line (reported / IFRS / non-IFRS) always labeled. |
| **FCF Definition Labeling** | Every FCF figure labeled **official vs aggregator-standard**; when they differ (e.g. 182.6B vs 215.6B), **report both with definitions** — never pick silently. Conflicting definitions surface as contested signals in Phase 7. |
| **Revenue/Turnover Scope Labeling** | Consolidated vs including-associates scope labeled per source (0001's ~280B vs ~507B trap) in `sources_index.md`, Phase 3, and the report. |
| **Price-Freshness Outcome in Outputs** | Phase 5 outputs state primary vs secondary closes, excluded stale sources, the primary source, and its as-of timestamp. |
| **Report Filename Single Source of Truth** | `report/<Code>-<Company>-<YYYY-MM-DD>-analysis.md` fixed in the Phase 1 output contract; on divergence the contract name wins and both are corrected. |
| **Review Reminder with Date AND Reason** | `task_state.json.review_reminder = {"date", "reason"}`; post-report lifecycle and completion checklist require both. |
| **HKEXnews Record-Once** | HKEXnews annual/interim PDFs recorded once as not machine-extractable; runs go straight to the logged aggregator-substitute ladder, no per-run re-attempt. |
| **Task Ticking + Canonical File Set** | Ticking at phase end also syncs `task_state.json` and fills change artifacts as phases run; superseded phase files are renamed `*.legacy.md`; main-spec requirement headers kept stable. |

#### v2.0 Highlights (history, retained)
...
│   └── state-management.md
├── stock-analysis/       # The single canonical single-stock analysis, HKEX-optimized (v2.1)
│   ├── SKILL.md
