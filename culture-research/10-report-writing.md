# Phase 10: Report Writing (renumbered in v3.3; REFINED in v3.2)
...
**Execution pattern:** Run as sub-sessions (SS15+ after the Phase 5 Audit Sub-session). One sub-agent per chapter/section, executed sequentially. Each agent writes to a shared directory.
...
**The most common failure in report writing is producing output that reads as a methodology report or academic literature review rather than a science story.** This happens because the synthesis document (Phase 8) is structured around the research process, and carrying that structure into the communication output shifts the subject from "what we discovered" to "how we collected."
...
This rule is the single most important structural difference between a synthesis document (Phase 8) and a communication output (Phase 10).
...
```
report/
├── report-charter.md         # editor-in-chief charter (see 09-sub-session-orchestration)
