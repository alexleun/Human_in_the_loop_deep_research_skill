# Phase 6: Multi-Round Iterative Analysis (renumbered in v3.3)
...
**Execution pattern:** For 5 rounds with substantial output each, run each round as a sub-session (SS8-SS12 after the Phase 5 Audit Sub-session). For smaller projects, can run in one session with sequential sub-agents.

**Input gate (NEW in v3.3):** This phase consumes the `stress-tested-matrix.json` produced by Phase 5 (Epistemic Stress-Test). The PM must verify the matrix exists and is schema-valid before dispatching any Reduce sub-session. Claims flagged `Unsubstantiated_Speculation` in the matrix are **prohibited** from being used as evidence in any round.
...
- Do NOT proceed to synthesis (Phase 8) without the checkpoint (Phase 7)
- Do NOT cite claims flagged as Unsubstantiated_Speculation in the stress-tested-matrix.json
