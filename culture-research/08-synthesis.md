# Phase 8: Synthesis Document (renumbered in v3.3)

**Purpose:** Integrate all 5 analysis rounds, the checkpoint review, and the Phase 5 stress-test matrix into a final synthesis document.

**Execution pattern:** Run as the final sub-session (SS14 after the Phase 5 Audit Sub-session). This is the terminal deliverable.

**Input gate (NEW in v3.3):** This phase reads `stress-tested-matrix.json` alongside the checkpoint review. The synthesis MUST honor the `synthesis_directives_for_phase_3` (retained key name; target is this Reduce phase) — specifically, claims flagged `Unsubstantiated_Speculation` in the matrix MUST NOT be cited as evidence in the synthesis document.
...
3. Read the batch notes of each round (SS8-SS12) for concise summaries
...
Prefer reading `knowledge-base/synthesis-input.json` if it exists (a pre-compiled aggregation of key data from all rounds). Read `stress-tested-matrix.json` (Phase 5) to confirm which claims are verified and which are excluded.
...
6. **Boundaries and Caveats** (from Checkpoint + Phase 5)
   - Critical missing dimensions
   - Underrepresented dimensions
   - Access distribution: X% of corpus is abstract-only
   - **Epistemic Limitations & Systemic Blindspots** (from Phase 5 stress-tested-matrix.json — mandatory in v3.3)
...
6. ✅ Boundaries section includes access distribution stats (X% abstract-only) AND Epistemic Limitations & Systemic Blindspots from the Phase 5 matrix
7. ✅ Research directions classified as evidence-derived / gap-derived / speculative
8. ✅ No Unsubstantiated_Speculation claims from stress-tested-matrix.json are cited
9. ✅ Batch note documents: section completion, top 5 research questions, missing dimensions flagged, Director Observations
10. ✅ The document opens cleanly (no broken `[[wikilink]]` blocks, no formatting issues)
...
- Do NOT cite Unsubstantiated_Speculation claims from the stress-tested-matrix.json
