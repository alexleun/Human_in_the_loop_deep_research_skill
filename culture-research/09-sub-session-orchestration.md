# Phase 9: Sub-Session Orchestration (renumbered in v3.3; REFINED in v3.0)
...
```json
{
  ...
  "scope": "global",
  "total_papers": 46,
  "phases_complete": ["explore", "search", "deep-read", "knowledge-base", "stress-test", "analysis-r1", "analysis-r2", "analysis-r3", "analysis-r4", "analysis-r5", "checkpoint", "synthesis"],
  ...
  "deliverables": {
    ...
    "stress_matrix": "stress-tested-matrix.json",
...
**Phase 5 state fields (NEW in v3.3):** After the Epistemic Stress-Test Audit Sub-session completes, add these fields to `project-state.json`:
...
```json
{
  "stress_test": {
    ...
    "stress_matrix_path": "stress-tested-matrix.json"
  }
}
```

These fields let downstream Reduce sub-sessions (Phase 6) and Synthesis (Phase 8) confirm the audit completed and honor its directives.
...
### 1. Project Context
- Project root path
- Skill file to load (e.g., `culture-research/03-deep-read.md`, or `culture-research/05-epistemic-stress-test.md` for the Audit SS)
- Source data paths
...
| SS3 | Epistemic Stress-Test (may be combined) | stress-tested-matrix.json |
| SS4 | Round 1–2 (thematic + comparison, combined) | Thematic map + matrix |
| SS5 | Round 3–4 (contradictions + gaps, combined) | Contradictions + gaps |
| SS6 | Round 5 + Checkpoint + Synthesis (combined) | Questions + checkpoint + synthesis |

Total: 6 sub-sessions. Rounds can be combined because fewer findings means smaller context per round. The stress-test is small enough to run inline for small projects.
...
| **SS7** | **Epistemic Stress-Test (NEW in v3.3)** | **stress-tested-matrix.json** |
| SS8 | Round 1 Thematic | round1-thematic-map.md |
| SS9 | Round 2 Comparison | round2-comparison-matrix.md |
| SS10 | Round 3 Contradictions | round3-contradictions.md |
| SS11 | Round 4 Gaps | round4-gaps.md |
| SS12 | Round 5 Questions | round5-research-questions.md |
| SS13 | Checkpoint | checkpoint-review.md |
| SS14 | Synthesis | synthesis.md |
| SS15+ | Report writing | report/ |
| SS16+ | Retrospective | skill-evolution-log.md |

Total: 14–16 sub-sessions. Each round gets its own session. Report writing is optional. The new SS7 Audit Sub-session is required before any Reduce dispatch.
...
- Relations: 1 SS (SS8)
- Epistemic Stress-Test: 1 dedicated Audit SS (SS9) — required before analysis
- Analysis: same 5 rounds, but each round may need sub-splitting (SS11a/SS11b/SS11c for Round 2)
- Report writing: 2+ SS if long-form article

Total: 19–23 sub-sessions.
...
- Epistemic Stress-Test: may need 2 Audit SS (one per region cluster) if the evidence base is very large
- Analysis rounds may each need 2-3 sub-sessions
- A dedicated PM sub-session may be needed for state tracking

Total: 26+ sub-sessions. Consider whether exhaustive coverage is necessary — 3-4 regions at 30-50 papers typically yields the best depth-to-coverage ratio.
