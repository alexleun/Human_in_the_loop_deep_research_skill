...
4. **Anti-lookahead audit (P9):** re-verify that no post-decision (post-cutoff) information was used to justify any position. Confirm every cited figure's as-of date ≤ decision date.
5. **Systems / emergence check (lightweight):** if quantitative data supports it, note reinforcing vs balancing feedback loops (e.g. network effects, inventory cycles, circular financing) and any time delays that could invalidate the thesis horizon. Optional scripted **Cognitive-Complexity / contradiction scan** over the evidence log.
6. **Reflector-style lessons (optional):** record what over-optimism or over-pessimism risks are present, to calibrate confidence in Phase 8.
...
4. ✅ Anti-lookahead audit passed — no post-decision evidence used (P9)
5. ✅ Fragility treatments are queued as inputs to Phase 4 / Phase 8 valuation and gates
6. ✅ Human has reviewed the audit for blind spots before Phase 8
